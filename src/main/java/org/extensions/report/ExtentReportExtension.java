...
public class ExtentReportExtension implements
    ...
    @Override public synchronized void beforeAll(ExtensionContext context) { this.createReport(context); }
    @Override public synchronized void beforeEach(ExtensionContext context) {this.createTest(context);}
    @Override public synchronized void afterEach(ExtensionContext context) {this.finishTest(context);}
    @Override public synchronized void testSuccessful(ExtensionContext context) {this.onTestPass(context);}
    @Override public synchronized void testDisabled(ExtensionContext context, Optional<String> reason) { this.onTestDisabled(context, reason); }
    @Override public synchronized void testAborted(ExtensionContext context, Throwable throwable) { this.onTestSkip(context, throwable); }
    @Override public synchronized void testFailed(ExtensionContext context, Throwable throwable) { this.onTestFail(context, throwable); }
    @Override public synchronized void afterAll(ExtensionContext context) {
    ...
    private synchronized void createTest(ExtensionContext context) {
        try {
            if (context.getElement().isPresent()) {
                ...
                if (testReportInfo.isPresent()) {
                    final String testMethod = context.getRequiredTestMethod().getName();
                    ExtentTestManager.createTest(testMethod, testReportInfo.get().assignCategory(), testReportInfo.get().assignAuthor());
    ...
    private synchronized void createMongoReport(ExtensionContext context) {
        try {
            if (context.getElement().isPresent()) {
                Optional<ReportConfiguration> configuration = this.readAnnotation(context, ReportConfiguration.class);
                if (configuration.isPresent()) {
                    this.reportConfigurations.get().setProperty("project.mongo.connection", configuration.get().mongoConnection());
